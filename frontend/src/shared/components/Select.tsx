    ...
    <div className={cn("w-full h-10 relative", wrapperClassName)}>
      ...
      <ShadCnSelect
        ...
        value={
          isEmptyValue
            ? NOT_CHOSEN_VALUE
            : String((value as T)?.[optionValueKey])
        }
      >
        <SelectTrigger
          className={cn(
            ...
            error &&
              "border-red-500 active:ring-red-500 active:outline-hidden active:ring-2 active:ring-offset-1 active:ring-offset-red-100 ",
            className
          )}
        >
          <SelectValue>
        ...
        <SelectContent>
          {showNotChosenText && (
            <SelectItem key="default" value={NOT_CHOSEN_VALUE}>
              {notChosenText}
            </SelectItem>
          )}
          {options.map(option => {
