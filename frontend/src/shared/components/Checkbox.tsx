...
interface CheckboxFieldProps
  extends Omit<CheckboxProps & RefAttributes<HTMLButtonElement>, "ref"> {
  name: string;
  label: ReactNode;
...
export const Checkbox: React.FC<CheckboxFieldProps> = ({
  ...
  >
    <ShadCnCheckbox id={name} name={name} className={className} {...props} />
    <Label htmlFor={name} className={cn("text-md font-medium", labelClassName)}>
      {label}
    </Label>
  </div>
);
